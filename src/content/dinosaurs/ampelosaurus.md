---
id: "ampelosaurus"
name: "アンペロサウルス"
kana: "あんぺろさうるす"
scientificName: "Ampelosaurus atacis"
era: "白亜紀後期"
region: "ヨーロッパ（フランス）"
diet: "草食"
length: "15.0m"
weight: "8.0t"
image: "images/ampelosaurus.webp"
group: "竜脚類"
description: "背中に骨質の装甲を持っていた竜脚類。ティタノサウルス類に属し、肉食恐竜から身を守るために皮膚に埋め込まれたトゲや板状の骨を発達させていた。"
bioTraits:
  - trait: "防御機構"
    value: "皮骨装甲"
    detail: "竜脚類としては珍しく高度に発達した装甲を持つ"
  - trait: "系統的特徴"
    value: "ティタノサウルス類"
    detail: "進化した竜脚類のグループに属し、広範囲に分布した"
cyberMetadata:
  bottomGap: 235
  realHeight: 8.5
  realLength: 15
  contentHeightPx: 551
  sourceRes: 1024
  dinoLeft: "5%"
  refRight: "20%"
  zoom: 0.9
---

白亜紀後期のヨーロッパ（フランス）に生息していた竜脚類（チタノサウルス類）。全長約15m。最大の特徴は、背中を覆う無数の骨質のトゲや円盤状の装甲プレートである。これによりヨーロッパに生息していたタラスコサウルスなどの大型肉食恐竜からの捕食攻撃を防いでいた。化石の脳腔のCTスキャン解析により、内耳が小さくバランス感覚があまり良くなかった（動きが非常に鈍かった）ことが判明している。
