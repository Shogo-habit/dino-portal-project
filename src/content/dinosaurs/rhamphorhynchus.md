---
id: "rhamphorhynchus"
name: "ランフォリンクス"
kana: "らんふぉりんくす"
scientificName: "Rhamphorhynchus muensteri"
era: "ジュラ紀後期"
region: "ヨーロッパ"
diet: "肉食（魚食）"
length: "翼開長1.2m"
weight: "2kg"
image: "images/rhamphorhynchus.webp"
group: "翼竜"
description: "長い尾の先端に菱形のひれを持つ翼竜。嘴には前方に突き出した鋭い歯があり、水面の魚を捕らえるのに適していた。"
bioTraits:
  - trait: "舵取り装置"
    value: "菱形の尾"
    detail: "尾の先端のひれが飛行中の方向転換を助ける"
  - trait: "捕食特化"
    value: "前向きの歯"
    detail: "滑りやすい魚を確実に捕らえる構造"
cyberMetadata:
  bottomGap: 272
  realHeight: 0.8
  realLength: 1.2
  contentHeightPx: 622
  sourceRes: 1024
  dinoLeft: "20%"
  refRight: "15%"
  zoom: 1
---

ジュラ紀後期のヨーロッパに生息していた、尾の長い原始的な翼竜（ラムフォリンクス類）の代表格。翼開長約1.26m。細長いクチバシには、前方を向いて噛み合う針のように鋭い歯が並んでいた。尾の先端には「菱形の舵（葉状のひれ）」があり、飛行時の空中バランスを保つ役割をしていた。化石の胃内容物から魚が発見されており、水面すれすれを飛びながら魚を捕らえていた。
