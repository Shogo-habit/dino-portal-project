---
id: "ankylosaurus"
name: "アンキロサウルス"
kana: "あんきろさうるす"
scientificName: "Ankylosaurus magniventris"
era: "白亜紀後期"
region: "北アメリカ"
diet: "草食"
length: "7.0m"
weight: "6.0t"
image: "images/dinosaurs/ankylosaurus/main.webp"
gallery:
  - "images/dinosaurs/ankylosaurus/movie.webp"
  - "images/dinosaurs/ankylosaurus/ecology.webp"
group: "よろい竜類"
description: "全身が装甲板で覆われ、尾の先端にはハンマーのような骨の塊がある。最も硬い恐竜の一つ。"
bioTraits:
  - trait: "装甲"
    value: "超硬質"
    detail: "皮膚に骨質の装飾が埋め込まれている"
  - trait: "尾の骨質ハンマー"
    value: "ハンマー"
    detail: "尾で肉食恐竜の骨を砕く"
cyberMetadata:
  bottomGap: 391
  realHeight: 1.7
  realLength: 7
  contentHeightPx: 242
  sourceRes: 1024
  dinoLeft: "0"
  refRight: "72%"
  zoom: 1.2
references:
  - title: "The Ankylosauridae, a new family of armored dinosaurs from the Upper Cretaceous"
    journal: "Bulletin of the American Museum of Natural History"
    year: 1908
    url: "https://hdl.handle.net/2246/1435"
---

白亜紀末期の北アメリカに生息した、植物食性の鎧竜。全身が装甲化された頑丈な骨質のプレート（皮骨）で覆われており、ティラノサウルスのような大型捕食者からの攻撃を完全に防御することができた。尾の先端には骨が癒合してできた重厚なハンマー（アンキロサウルス・クラブ）を備えており、これを左右に激しく振ることで、天敵の脚骨を粉砕する致命的な反撃手段として使用した。低く扁平な体つきをしており、地表付近の低木類を主食としていた。
